...
dataset = WikipediaNetwork(path, args.dataset, geom_gcn_preprocess=True, transform=T.NormalizeFeatures())
data = dataset[0]
...
for lr in lrs:
    ...
    def train():
        ...
        optimizer.zero_grad()
        # data.x: [2277, 2325]
        # data.edge_index: [2, 36101]

        out = model(data.x, data.edge_index, data.edge_attr)  # [2277, 5]
        # data.train_mask: [2277, 10] 왜??????????? 10은 뭐지?????????cora는 1이었음...
